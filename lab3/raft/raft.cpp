...
void Raft::handle_election_timeout_(const boost::system::error_code & ec){
...
void Raft::handle_heart_beat_timeout_(const boost::system::error_code & ec){
    if(ec == boost::asio::error::operation_aborted) return;
    send_append_entries_();
    restart_heart_beat_timer_();
}
...
void Raft::start_election_(){
    ...
    client_->send_request_vote_to_all(std::move(req));
}

void Raft::send_append_entries_(){
    for(int peer_id = 0; peer_id < num_peers_; ++peer_id){
        if(peer_id == me_) continue;
        if(in_flight_[peer_id]) continue;
        send_append_entries_with_id_(peer_id);
        dirty_[peer_id] = false;
    }
}

void Raft::send_append_entries_with_id_(int peer_id){
    ...
    client_->send_append_entries_with_id(std::move(req),peer_id,entries_.size()-1);
}

void Raft::become_follower_(){
    heart_beat_timer_.cancel();
    role_ = RaftRole::FOLLOWER;
    restart_election_timer_();
}

void Raft::become_candidate_(){
    heart_beat_timer_.cancel();
...
void Raft::restart_election_timer_(){
    election_timer_.expires_from_now(std::chrono::milliseconds(distrib_(gen_)));
    election_timer_.async_wait(std::bind(&Raft::handle_election_timeout_,this,std::placeholders::_1));
}

void Raft::restart_heart_beat_timer_(){
    heart_beat_timer_.expires_from_now(std::chrono::milliseconds(300));
    heart_beat_timer_.async_wait(std::bind(&Raft::handle_heart_beat_timeout_,this,std::placeholders::_1));
}
...
Raft::Raft(std::vector<std::string> peer_address, int me):
        me_(me),
        ...
        server_(std::make_unique<RaftRpcServer>(peer_address[me],this)),
        client_(std::make_unique<RaftRpcClient>(peer_address,me,this)),
...
void Raft::on_request_vote_reply(const RequestVoteReply& rep, int peer_id){
    ...
    if(gained_vote_ > num_peers_/2) become_leader_();
    return;
}

void Raft::start(const std::string command){
    if(role_ != RaftRole::LEADER) return;
